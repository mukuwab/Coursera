...
- useState: The useState hook allows you to add state to functional components in React, enabling the tracking of values that can change over time.

- Local State: Local state refers to state variables that are specific to a component and cannot be accessed from outside that component.

- Hooks Rules: Hooks must be called at the top level of a component, cannot be called inside loops or conditions, and can only be called from React functions.

- useRef: The useRef hook provides a way to access and interact with DOM elements directly, allowing for operations like focusing an input field.
