...
- Client-Side Rendering: Client-side rendering involves downloading an index.html file and allowing React to inject its code into a dedicated HTML element, which is the approach used in this course.

- Code-Splitting: Code-splitting is a technique where a module bundler splits a single bundle into multiple bundles, loading them as needed to improve app performance.

- Server-Side Rendering (SSR): SSR is a technique where React components are rendered to HTML on the server, allowing the visitor to download the finished HTML code, improving initial load times.

- Webpack: Webpack is a module bundler that takes various file types and bundles them into a single file for browser consumption, optimizing the loading process.

- Dependency Graph: A dependency graph is a representation of how different files in a project depend on each other, which is crucial for proper bundling.
