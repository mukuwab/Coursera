...
- JSX: A syntax extension for JavaScript that allows writing HTML-like code within JavaScript, which is then transformed into React elements.

- Ternary Operator: A shorthand syntax in JavaScript that allows for conditional expressions in the form of 'condition ? trueValue : falseValue'.
- Function Invocation in JSX: The process of calling a function within JSX to return a value, which can be displayed dynamically in the rendered output.
