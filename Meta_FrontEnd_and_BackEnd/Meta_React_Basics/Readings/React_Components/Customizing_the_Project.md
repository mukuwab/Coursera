# Key Concepts
- JSX Syntax: JSX is a syntax extension for JavaScript that resembles HTML, used in React to describe the UI structure, with 'className' replacing 'class'.

- Functional Components: Functional components are JavaScript functions that return JSX, allowing for the creation of reusable UI elements in React.

- DRY Principle: The 'Don't Repeat Yourself' principle encourages the reuse of code to avoid redundancy, which can be achieved through props in React components.

- Folder Structure: A well-organized folder structure is crucial for managing components and files in a React project, typically involving a 'components' folder.
...
This is where 
React docs
 can help. They suggest two approaches:
