# Key Concepts
GET Method: The GET method appends form data to the URL, making it visible in the browser's address bar, suitable for non-sensitive data.

POST Method: The POST method sends form data in the body of the HTTP request, keeping it hidden from the URL, making it suitable for sensitive data.

Action Attribute: The action attribute specifies the web address to which the form data will be sent, which can be a full URL, absolute path, or relative path.

Method Attribute: The method attribute indicates the HTTP method (GET or POST) used to submit the form, with GET encoding data in the URL and POST sending data in the request body.

Absolute and Relative Paths: Absolute paths start with a forward slash and use the base URL, while relative paths combine the current URL with the specified path.
