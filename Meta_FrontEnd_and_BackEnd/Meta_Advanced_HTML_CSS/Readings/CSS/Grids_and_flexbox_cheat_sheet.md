...
Flexbox: A one-dimensional layout model that enables the arrangement of items in a row or column, allowing for flexible and responsive designs.

grid-template-rows: Defines the size of the rows in a grid layout, allowing for customization of row heights.

flex-direction: Specifies the direction in which flex items are placed in the flex container, affecting the layout flow.

justify-content: Aligns flex items along the main axis, controlling the distribution of space between items.

CSS Grid: A layout system that allows for the arrangement of elements in rows and columns, providing control over the layout structure.
