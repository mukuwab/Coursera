...
# Radio Button: 
Displays a radio button that allows only a single value to be selected from multiple choices.

# File Input: 
Displays a control for selecting and uploading files from the user's computer.

# Text Input: 
Defines a basic single-line text field for user input.
# Password Input: 
Defines a single-line text field for sensitive information, obscuring the input value.

# Email Input: 
Defines a field for entering an email address, with automatic validation upon submission.

# Button: 
Displays a clickable button used in HTML forms to activate scripts when clicked.

# Date Input: 
Displays a control for entering a date without time.
